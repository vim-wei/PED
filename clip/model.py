...
class ResidualAttentionBlock_IVLP(nn.Module):
        ...
        if i != 0:
            ...
            if self.add_prompt:
                if self.text_layer:
                    ...
                    ctx_vectors = torch.empty(design_details["expert"], self.n_ctx_text, d_model)
                else:
                    self.n_ctx_visual = design_details["vision_ctx"]  # hyperparameter
                    ctx_vectors = torch.empty(design_details["expert"], self.n_ctx_visual, d_model)
    ...
    def forward(self, x: torch.Tensor, index=None, merge=False):
        if merge:
            if self.add_prompt:
                if not self.text_layer:
                    prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :]
                    visual_context = self.VPT_shallow.mean(dim=0).expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                    x = torch.cat([prefix, visual_context], dim=0)
                else:
                    prefix = x[:1, :, :]
                    suffix = x[1 + self.n_ctx_text:, :, :]
                    textual_context = self.VPT_shallow.mean(dim=0).expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
            ...
            if self.add_prompt:
                ...
                if not self.text_layer:
                    ...
                    visual_context = self.VPT_shallow[index].expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                    ...
                    suffix = x[1 + self.n_ctx_text:, :, :]
                    # Create/configure learnable tokens of this layer
                    textual_context = self.VPT_shallow[index].expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT' or current_trainer == 'PED':
    ...
    def forward(self, x: torch.Tensor, index=None, merge=False):
        ...
        for _, blk in enumerate(self.resblocks):
            x = blk(x, index, merge=merge)
        return x
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        if self.VPT_shallow:
            ...
            ctx_vectors = torch.empty(design_details["expert"], n_ctx, width)
            nn.init.normal_(ctx_vectors, std=0.02)
        ...
        self.expert = design_details["expert"]
    ...
    def forward(self, x: torch.Tensor, index=None, merge=False):
        ...
        if self.expert == 0:
            ...
            if merge:
                visual_ctx = self.VPT.mean(dim=0).expand(x.shape[0], -1, -1).half()
                ...
                if index is not None:
                    visual_ctx = self.VPT[index].expand(x.shape[0], -1, -1).half()
                    every_x = torch.cat([x, visual_ctx], dim=1)
                    every_x = self.ln_pre(every_x)
                    every_x = every_x.permute(1, 0, 2)  # NLD -> LND
                    every_x = self.transformer(every_x, index=index)
                    every_x = every_x.permute(1, 0, 2)  # LND -> NLD
                    every_x = self.ln_post(every_x[:, 0, :])
                    if self.proj is not None:
                        every_x = every_x @ self.proj
                    
                    return every_x
                else:
                    all_x = []
                    for i in range(self.expert):
                        visual_ctx = self.VPT[i].expand(x.shape[0], -1, -1).half()
                        every_x = torch.cat([x, visual_ctx], dim=1)
                        every_x = self.ln_pre(every_x)
                        every_x = every_x.permute(1, 0, 2)  # NLD -> LND
                        every_x = self.transformer(every_x, index=i)
                        every_x = every_x.permute(1, 0, 2)  # LND -> NLD
                        every_x = self.ln_post(every_x[:, 0, :])
                        if self.proj is not None:
                            every_x = every_x @ self.proj
                        all_x.append(every_x)
